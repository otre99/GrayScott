...
ImageViewer::ImageViewer(QWidget *parent) : QAbstractScrollArea(parent) {
  ...
  setColormap(ColorMapper::gpCold);
}

void ImageViewer::updateData(const std::vector<double> &data) {
  m_rawData = data;
  const int dim = static_cast<int>(sqrt(data.size()));
  const QSize newSize(dim, dim);
  bool new_image = false;
  if (newSize != m_imageData.size()) {
    m_imageData = QImage(newSize, QImage::Format_ARGB32_Premultiplied);
    new_image = true;
  }
  colorize();

  if (new_image) {
    attachImagePtr(&m_imageData);
  } else {
    m_cachedPixmap = {};
    viewport()->update();
  }
...
void ImageViewer::setColormap(ColorMapper::GradientPreset p) {
  m_colorMapper = ColorMapper(p);
}

void ImageViewer::colorize() {
  const int dim = static_cast<int>(sqrt(m_rawData.size()));
  double lower = std::numeric_limits<double>::max();
  double upper = std::numeric_limits<double>::min();
  for (const auto &v : m_rawData) {
    lower = std::min(lower, v);
    upper = std::max(upper, v);
  }
  for (int i = 0; i < m_imageData.height(); ++i) {
    m_colorMapper.colorize(m_rawData.data() + i * dim, lower, upper,
                           reinterpret_cast<QRgb *>(m_imageData.scanLine(i)),
                           dim, 1, m_useLog);
  }
}

void ImageViewer::forceToRepaint() {
  m_cachedPixmap = {};
  viewport()->update();
}
...
void ImageViewer::selectScf() {
  ...
  if (m_scaleFactor == 0.0) m_scaleFactor = 1.0;
}
